...
One config file per printer, named `config/<printer>_config.json` (currently just `bambu_p1s_config.json`). Loaded via `commentjson` (supports `//` comments). `machine` (startPos/penOffset/plateSize/safeZoneSize/safeZoneOffset/canvasSize/canvasOffset — see Alignment & Scaling below), `processing` (what is drawn on paper: tessellationTolerance, infillSpacing, prefix/suffix gcode template paths), `motion` (how the pen moves while drawing: per-role heights/speeds/accels, shortTravelThreshold, loadDelay), `visualization` (pen width, cosmetic layering/coloring for Bambu Studio preview), `debug` (showBoundingBoxes, optimizePathOrder, profiling). All fields are type-checked before use; on mismatch, the setting is skipped and a warning is printed. In `motion.heights`/`speeds`/`accels` (and `visualization.lineTypes`), a `"draw"` key sets all three draw roles at once; an explicit `perimeter`/`infill`/`gapInfill` key overrides `"draw"` for just that role — this expansion happens once in `initFromJson`, so the rest of the pipeline just indexes by concrete `LineType`. `initFromJson` ends with `_validate()`, which calls `_validateBounds()` (warn-and-continue, never resets to defaults — see Alignment & Scaling) and also warns if `generateGapInfill` is enabled while `infillSpacing <= 0` disables infill entirely.
