...
| `structure-misc` | nested groups, fill inheritance + override, `<use>`/`<symbol>` `[DROP]`, clipPath/mask/pattern `[PARTIAL]`, `display:none` & `visibility:hidden` ⚠️ *drawn anyway — see Known gaps*, opacity `[PARTIAL]`, a 7-segment polygon (`heptagon-segment-color-wrap`) whose segment count doesn't divide evenly into `visualization.segmentTypes`' length — exercises `lib/plot.py`'s `_skipRepeatedClosingColor` fix for `style: "segment"`'s closing-color-repeat bug |
