...
| `test_stroke.py` | per-module | `_passDeltas`' pass-offset arithmetic (even pitch, outermost pass landing exactly on `strokeWidth/2`, both parities), and the dash walk: inked length equals the pattern's duty cycle, odd-length patterns repeat per spec, `dashoffset` shifts the pattern (negative shifts backwards) without changing total ink on a closed loop, the seam dash merges across a closed path's start point, and arcs survive dashing. Pipeline-level: dashes ink less than solid and never outside the solid band, the `generateStroke=False` fallback still dashes, and a dashed stroke over a fill draws **only its outward half** — with the two guards that it stays two-sided when there's no fill and when the shape encloses no area |
