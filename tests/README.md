...
| `test_calibration.py` | per-module | The calibration-sheet framework: `promptNumber`/`promptRamp` re-asking on non-numeric, empty, non-finite, out-of-range, non-positive-max-minus-min, zero-step, and `boundsCheck`-rejected input (the repo's only coverage of an `input()` path); `_rampValues`' inclusive-of-`hi`-when-it-divides-evenly, never-overshoots-otherwise arithmetic; `_translatePaths`/`_centerPasses` moving a pattern's geometry and label origin together; the stroke font's per-glyph bounds and fixed per-character advance; `generateCalibration` turning each `Pass` into a `PathObject` carrying only the overrides that pass actually set, giving a label object the same overrides as its row but in its own dict, and producing a `Document` the real `createFile` writes with no calibration-specific path; and `calibrationEnabled` staying silent on `"none"` while rejecting an unregistered name and listing the registered ones. `_rulerSweep` (shared by `height`/`speed`): one pass per ramp value, ascending values stacking bottom-to-top at a `penWidth`-scaled pitch, every 5th/10th row ruler-ticked longer, only the 10th carrying a label (to its right, vertically centered on the row), alternating draw direction row to row, every point landing inside the canvas bounds, a caller-supplied multi-segment shape tiling the same way a plain line does, and the supplied shapes never being mutated (each row gets its own copy). `height`/`speed` each get a thin test of their own override/label/row-length choice, plus a `generateCalibration` round trip. A separate sanity check parses every shipped `config/machines/*.json` and fails (naming the file) if any ships with `calibrationTest` left set to something other than `"none"`. Remaining tests (accel, ...) get their own coverage as each is added |
