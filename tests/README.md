...
| `test_calibration.py` | per-module | The calibration-sheet framework: `promptNumber`/`promptRamp` re-asking on non-numeric, empty, non-finite, out-of-range, non-positive-max-minus-min, zero-step, and `boundsCheck`-rejected input (the repo's only coverage of an `input()` path); `_rampValues`' inclusive-of-`hi`-when-it-divides-evenly, never-overshoots-otherwise arithmetic; `_translatePaths`/`_centerPasses` moving a pattern's geometry and label origin together; the stroke font's per-glyph bounds and fixed per-character advance; `generateCalibration` turning each `Pass` into a `PathObject` carrying only the overrides that pass actually set, giving a label object the pass's height but *not* its speed/accel, and producing a `Document` the real `createFile` writes with no calibration-specific path; and `calibrationEnabled` staying silent on `"none"` while rejecting an unregistered name and listing the registered ones. `height`: one pass per ramp value with the value as its own override, ascending values stacking bottom-to-top at `2*penWidth` pitch, every 5th/10th line ruler-ticked longer, only the 10th carrying a label to its right, and every point landing inside the canvas bounds. A separate sanity check parses every shipped `config/machines/*.json` and fails (naming the file) if any ships with `calibrationTest` left set to something other than `"none"`. Remaining tests (speed, accel, ...) get their own coverage as each is added |
