...
| `test_coverage.py` | cross-cutting | The coverage invariant: for every object, the region SVG would paint (fill region + stroke band, recomputed independently of `generateInfill`) minus every drawn centerline swept by ±`fillSpacing/2` leaves nothing thicker than `GAP_HALF_WIDTH_FRACTION` — a known-issues baseline sitting just above today's worst gap. Also asserts the reverse direction — no gap-fill centerline's ±`penWidth/2` of ink lands outside the shape it belongs to — and that disabling `generateGapInfill` measurably worsens coverage, so the residue pass can't become a silent no-op. Marked `slow` |
...
| `test_settings.py` | per-module | `Settings.initFromMachineJson`/`initFromSlicerJson`: fallback to defaults on a missing/malformed/wrong-shape file, the containment warnings (machine side only), `"draw"` expansion and per-role override, the mm/s→mm/min conversions, alignment resolution to a lower-left offset, that every wrongly-typed setting is both skipped *and* reported, that `_MACHINE_FIELDS`/`_SLICER_FIELDS` partition every `Settings` field with none left over, and that a field loaded from the wrong side's file is reported with a "belongs in the other config" hint rather than a plain unknown-setting warning. A draw height configured above the travel height warns (an override doing the same is `test_plot.py`'s). The three debug bounding-box heights (`_segmentBounds`/`_pathBounds`/`_documentBounds`) are derived from `max(stroke, infill, gapInfill)` rather than configurable — a leftover value for one in the JSON is rejected with a warning naming it. Config fixtures live in `configs/` |
| `test_calibration.py` | per-module | The calibration-sheet framework: `promptNumber`/`promptRamp` re-asking on non-numeric, empty, non-finite, out-of-range, non-positive-max-minus-min, zero-step, and `boundsCheck`-rejected input (the repo's only coverage of an `input()` path); `_rampValues`' inclusive-of-`hi`-when-it-divides-evenly, never-overshoots-otherwise arithmetic; `_translatePaths`/`_centerPasses` moving a pattern's geometry and label origin together; the stroke font's per-glyph bounds and fixed per-character advance; `generateCalibration` turning each `Pass` into a `PathObject` carrying only the overrides that pass actually set, giving a label object the same overrides as its row but in its own dict, and producing a `Document` the real `createFile` writes with no calibration-specific path; and `calibrationEnabled` staying silent on `"none"` while rejecting an unregistered name and listing the registered ones. `_rulerSweep` (shared by `height`/`speed`/`accel`): one pass per ramp value, ascending values stacking bottom-to-top at a pitch that grows with the row shape's own height (a flat line collapses to the old fixed `penWidth`-scaled pitch; a tall shape like `accel`'s zigzag gets more clearance so it can't overlap the row above it), every 5th/10th row ruler-ticked longer, only the 10th carrying a label (to its right, vertically centered on the row), alternating draw direction row to row, every point landing inside the canvas bounds, a caller-supplied multi-segment shape tiling the same way a plain line does, and the supplied shapes never being mutated (each row gets its own copy). `height`/`speed`/`accel` each get a thin test of their own override/label/row-length choice, plus a `generateCalibration` round trip. `accel`'s zigzag geometry gets its own coverage: `_zigzagLeg` alternating up/down from a given start point and direction (and handing back the direction the next leg should continue with), and `_zigzagShapes` chaining a wide leg into a narrow one into one continuous stroke starting at the bottom (a straight line can't show an acceleration problem, since nothing on it changes direction) whose ruler ticks are 1-2 more narrow-pitch teeth rather than a plain dash. A separate sanity check parses every shipped `config/machines/*.json` and fails (naming the file) if any ships with `calibrationTest` left set to something other than `"none"`. Remaining tests (spacing, edge, ...) get their own coverage as each is added |

Config fixtures for `test_settings.py` live in `configs/`: `invalid.json`
(malformed), `not-sections.json` (valid JSON, wrong shape), and
`wrong-types-machine.json`/`wrong-types-slicer.json` (every setting named
correctly and typed wrongly, split by which loader owns those fields). Cases
that are about a *value* rather than the file are built inline in the test.
